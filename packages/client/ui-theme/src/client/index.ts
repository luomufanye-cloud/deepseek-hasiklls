...
import { wallpaperBackground } from './wallpaper-catalog.ts'
import {
  DEFAULT_PREFERENCE, DEFAULT_WALLPAPER, isThemePreference, isWallpaperId, THEME_PREFERENCE_FIELD,
  THEME_SETTINGS_NAMESPACE, WALLPAPER_FIELD,
  type ThemePreference, type ThemeSettings,
} from '../theme-settings.ts'

export type { AppearanceRowComponentProps, AppearanceRowInjected } from './AppearanceRow.tsx'
export type { WallpaperRowComponentProps, WallpaperRowInjected } from './WallpaperRow.tsx'
export type { WallpaperPreset } from './wallpaper-catalog.ts'
export type { AppearanceRowState } from './settings-store.ts'
export type { ThemeKey } from './locales.ts'
export type { ThemePreference, ThemeSettings, WallpaperId } from '../theme-settings.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  // Settings-row state mirror: both rows (Appearance and Wallpaper) read the
  // same store. Preference is mirrored from the theme service (synchronous and
  // authoritative); wallpaper comes from the settings scope, whose publication
  // is asynchronous — so sync uses a plugin-owned monotonic counter as the
  // store revision rather than trusting either service's counter timing. Every
  // sync call therefore lands; duplicates merely re-apply identical values.
  ...
  const syncStore = (): void => {
    ...
    bound?.sync(
      ...
      isWallpaperId(host.getSnapshot().value?.wallpaper) ? host.getSnapshot().value!.wallpaper : DEFAULT_WALLPAPER,
      storeRevision,
    )
  }
  ctx.on('theme/change', () => { syncStore() })
  // A wallpaper write (or any ui-theme settings change) lands through the
  // settings scope asynchronously; mirror it into the row store so the
  // Wallpaper row re-renders its selection.
  ctx.effect(() => { return host.subscribe(() => { syncStore() }) }, 'ui-theme: settings row store sync')
  ...
  const wallpaperInjected = (actions: BoundActions<typeof store>): WallpaperRowInjected => {
    bound = actions
    syncStore()
    return {
      setWallpaper: (id) => { if (isWallpaperId(id)) void host.set(WALLPAPER_FIELD, id) },
    }
  }
  ctx.slots.inject('settings.general.item', () => ctx.slots.register({
    name: 'settings.general.item',
  ...
  // Wallpaper DOM presenter: paints the selected art behind the application by
  // exposing a CSS custom property (--dsw-wallpaper) on <body> and an
  // html[data-dsh-wallpaper="<id>"] attribute for ui-layout's frame surfaces.
  // Purely DOM writes, decoupled from the theme snapshot; `none` retracts both.
  // Node test environments have no document, so the effect is a no-op there
  // (the model still validates through the settings schema).
  ctx.effect(() => {
    ...
    const applyWallpaper = (): void => {
      ...
      const id = isWallpaperId(section?.wallpaper) ? section.wallpaper : DEFAULT_WALLPAPER
      body.style.setProperty('--dsw-wallpaper', wallpaperBackground(id))
      if (id === 'none') root.removeAttribute('data-dsh-wallpaper')
      else root.setAttribute('data-dsh-wallpaper', id)
    }
    applyWallpaper()
    const off = host.subscribe(applyWallpaper)
    return () => { off() }
  }, 'ui-theme: wallpaper presenter')
}
