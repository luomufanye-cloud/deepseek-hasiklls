...
import {
  DEFAULT_PREFERENCE, DEFAULT_WALLPAPER, THEME_SETTINGS_NAMESPACE, apply,
...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({ preference: DEFAULT_PREFERENCE, wallpaper: DEFAULT_WALLPAPER })
    await ctx.settings.update(ns, { preference: 'dark' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'dark', wallpaper: DEFAULT_WALLPAPER })
    await expect(ctx.settings.update(ns, { preference: 'sepia' })).rejects.toThrow()
    await expect(ctx.settings.update(ns, { wallpaper: 'quilt' })).rejects.toThrow()
    await fiber.dispose()
